...
from analyse.analyze_consensusHMM_revision import analyze_consensusHMM
...
def main():
	...
	for j in range(sim_range):
		if change == 'changeN':
			...
			prop = 0.5  # proportion of fast. ONLY USED FOR STAT HET
			index = prop
			print("prop = {}".format(prop))
		elif change == 'changerate':
			tsh = 10 # metric of transition between slow and fast. ONLY USED FOR DYN HET
			index = tsh
	...
	pickle.dump(Edict, open( "figures/results/ACFdicts_rev2_{}_{}_{}_{}_{}.p".format(nstates,dataset,snr,model,change), "wb" ))
	pickle.dump(resdict, open( "figures/results/Resultdicts_rev2_{}_{}_{}_{}_{}.p".format(nstates,dataset,snr,model,change), "wb" ) )
